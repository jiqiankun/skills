...
const DAYS = [
  {
    ...
    // —— POI 列表（时间轴）——
    // 注意：日级不设任何摄影字段（无「今日必拍」/摄影时间轴/摄影路线）。摄影信息只挂在 POI 的 photo_info 上。
    pois: [ /* 见下 */ ],
    // —— 备用方案（4 个折叠）——
    spare: {
      complete: "完整路线…………",
...
```js
{
  ...
  photo: "五花海观景台、镜海倒影(无风晨间)",        // 拍照一句话摘要（兼容旧字段；有 photo_info 时可省略）

  // —— 📷 拍照打卡（photo_info；可选！行程锁定后再补充，只作为地点的附属信息）——
  // 只有真正值得拍的地点才写（古村/古镇/建筑/山水/观景台/花海梯田/老街/夜景/有特色的景观餐厅酒店）；
  // 停车场、交通中转、服务区、普通餐厅等不要写。1–2 个机位即可，模块不能比景点介绍长。
  photo_info: {
    spots: [
      { location: "五花海观景台西侧木栈道",   // 机位：景区内部具体位置（一句话）
        subjects: ["倒影","湖水","彩林"],      // 拍什么标签（≤4 个）：人像/建筑/古巷/梯田/山景/晨雾/溪流/倒影/日落/夜景/街拍/美食…
        tip: "手机 1x 拍湖景倒影，2x 压缩彩林层次；无风时等倒影完整再按快门"  // 一句拍法（手机优先：倍率/构图/站位），不写专业参数
      }
    ],
    best_time: "无风清晨倒影最完整",        // 参考拍摄时间，仅供参考
    // 当前计划说明（两种写法）：
    // ① 无需调整：「当前安排 14:00–16:00 游览，下午侧光也能正常出片，无需调整」
    // ② 可选微调（≤30 分钟、不动景点/顺序/餐住/交通衔接）：
    //    「想拍日落前柔光，可把到达从 16:30 微调至 17:00（约 +30 分钟，不影响 18:30 晚餐）；按原时间到也能拍」
    // 超过 30 分钟 / 换点 / 改顺序 / 动餐宿 = 过度调整，禁止；只能写「当前时段怎么拍好」。
    schedule_note: "当前安排 14:00–16:00 游览，下午侧光也能正常出片，无需为摄影调整行程"
  },
...
```js
{
  ...
  // 景观餐厅/有视觉特色的店可挂 photo_info（「📷 顺手拍」：1 个机位 + 一句拍法，如「二楼露台看古村屋顶」），
  // 但不能因为拍照换餐厅/改用餐时间。
}
```
...
| `photo_info{}` | 📷 可选拍照打卡（行程锁定后补充）：spots[]{location 机位 / subjects[] 主题标签 / tip 一句手机拍法}、best_time 参考时间、schedule_note 当前计划说明（可含 ≤30 分钟可选微调，禁止过度调整） | 值得拍的地点（含景观餐厅/民宿），1–2 个机位 |
